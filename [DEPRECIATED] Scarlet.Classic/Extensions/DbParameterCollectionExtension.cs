...
namespace Scarlet.Extensions
{
    public static class DbParameterCollectionExtension
    {
        public static void AddInput<TParameter>(this ICollection<TParameter> parameters, string parameterName, object value)
            where TParameter : DbParameter, new()
        {
            if (string.IsNullOrWhiteSpace(parameterName))
            {
                throw ExceptionInstantiator.NullOrWhiteSpaceArgument(nameof(parameterName));
            }

            parameters.Add(new TParameter()
            {
        ...
        public static void AddInputOut<TParameter>(this ICollection<TParameter> parameters, string parameterName, object value)
           where TParameter : DbParameter, new()
        {
            if (string.IsNullOrWhiteSpace(parameterName))
            {
        ...
        public static void AddOutput<TParameter>(this ICollection<TParameter> parameters, string parameterName)
            where TParameter : DbParameter, new()
        {
            if (string.IsNullOrWhiteSpace(parameterName))
            {
        ...
        private static void ValidateGetterArguments(DbParameterCollection parameters, string parameterName)
        {
            if (string.IsNullOrWhiteSpace(parameterName))
            {
                ExceptionInstantiator.NullOrWhiteSpaceArgument(nameof(parameterName));
            }

            if (parameters.Count <= 0)
            {
                ExceptionInstantiator.EmptyCollectionArgument(nameof(parameters));
            }
        }
